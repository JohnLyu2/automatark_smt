...
; ^(\d|,)*\d*$
(assert (not (str.in_re X (re.++ (re.* (re.union (re.range "0" "9") (str.to_re ","))) (re.* (re.range "0" "9")) (str.to_re "\u{a}")))))
; www\.take5bingo\.com\d+Host\x3AHost\x3A
(assert (str.in_re X (re.++ (str.to_re "www.take5bingo.com\u{1b}") (re.+ (re.range "0" "9")) (str.to_re "Host:Host:\u{a}"))))
